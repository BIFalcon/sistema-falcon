...
DECLARE
  ...
  v_count integer := 0;
BEGIN
  -- 1) Cartão: adquirente x front caixa (data + categoria + valor, 1:1 exato)
  FOR r IN
    WITH a AS (
      SELECT hotel_id, sale_date AS d, lower(btrim(categoria)) AS cat, amount,
             min(id) AS id, count(*) AS c
  ...
  -- 2) PIX: front caixa x extrato bancário (data + valor, 1:1 exato)
  FOR r IN
    WITH o AS (
      SELECT hotel_id, business_date AS d, amount, min(id) AS id, count(*) AS c
      FROM public.conc_opera_entries
      WHERE matched_at IS NULL AND business_date IS NOT NULL
      ...
      SELECT hotel_id, line_date AS d, amount, min(id) AS id, count(*) AS c
