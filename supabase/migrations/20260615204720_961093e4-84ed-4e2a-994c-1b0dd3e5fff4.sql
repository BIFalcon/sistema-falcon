...
BEGIN
  ...
  -- ===== CARTA — novo fluxo com aprovação intermediária do GOP =====

  -- GG aprovou → aguardando_gop: notifica GOP (SLA 24h) e RI
  IF NEW.status_carta = 'aguardando_gop' AND OLD.status_carta = 'aguardando_gg' THEN
    SELECT COALESCE(jsonb_agg(jsonb_build_object('user_id', user_id, 'email', email, 'role', 'gop')), '[]'::jsonb)
      INTO v_recipients FROM public.users_with_role_for_hotel('gop', NEW.hotel_id);
    v_subject := '[' || v_hotel.name || '] Carta aprovada pelo GG — aguardando GOP — ' || v_period;
    v_body := 'O GG aprovou a Carta ao Investidor de **' || v_hotel.name || '** (' || v_period || ').' || E'\n\n' ||
      'Você tem **24 horas** para revisar, ajustar se necessário e aprovar — ou devolver com comentários.' || E'\n\n' ||
  ...
  -- GOP aprovou → aguardando_fernando: notifica Fernando (SLA 24h), RI e (cópia) GOP
  IF NEW.status_carta = 'aguardando_fernando' AND OLD.status_carta = 'aguardando_gop' THEN
    SELECT COALESCE(jsonb_agg(jsonb_build_object('user_id', user_id, 'email', email, 'role', 'fernando')), '[]'::jsonb)
      INTO v_recipients FROM public.users_with_role_global('fernando');
    v_subject := '[' || v_hotel.name || '] Carta aguardando revisão final — ' || v_period;
    v_body := 'A Carta de **' || v_hotel.name || '** (' || v_period || ') foi aprovada pelo GOP e aguarda sua revisão final.' || E'\n\n' ||
    ...
    SELECT COALESCE(jsonb_agg(jsonb_build_object('user_id', user_id, 'email', email, 'role', 'ri')), '[]'::jsonb)
      INTO v_recipients FROM public.users_with_role_global('ri');
    v_subject := '[' || v_hotel.name || '] Carta aprovada pelo GOP — ' || v_period;
    v_body := 'O GOP aprovou a Carta de **' || v_hotel.name ||
      '** (' || v_period || '). Aguardando revisão final do Fernando.' ||
      E'\n\n' || '[Ver Carta](' || v_link_carta || ')';
    PERFORM public.enqueue_workflow_notification('carta_gop_approved', NEW.id, NEW.hotel_id, v_recipients, v_subject, v_body, v_link_carta,
      jsonb_build_object('audience', 'ri'));
    ...
    SELECT COALESCE(jsonb_agg(jsonb_build_object('user_id', user_id, 'email', email, 'role', 'gop')), '[]'::jsonb)
      INTO v_recipients FROM public.users_with_role_for_hotel('gop', NEW.hotel_id);
    v_subject := '[' || v_hotel.name || '] Carta encaminhada ao Fernando — ' || v_period;
    v_body := 'A Carta de **' || v_hotel.name || '** (' || v_period || ') foi encaminhada para aprovação final do Fernando.' ||
      E'\n\n' || 'Você ainda pode editar até a aprovação final.' ||
      E'\n\n' || '[Abrir Carta](' || v_link_carta || ')';
    PERFORM public.enqueue_workflow_notification('carta_gop_approved', NEW.id, NEW.hotel_id, v_recipients, v_subject, v_body, v_link_carta,
      jsonb_build_object('audience', 'gop'));
  END IF;

  -- Fernando aprovou → aprovado
